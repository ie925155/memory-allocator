...
void* mem_sbrk(intptr_t incr) {
  ...
  if (incr < 0) {
    ...
    fprintf(stderr,
            "ERROR: mem_sbrk failed. Ran out of memory.  Would require heap size of %zd (0x%zx) "
            "bytes\n",
            alloc, alloc);
...
uint64_t mem_read(const void* addr, size_t len) {
  uint64_t rdata;
  if (sparse && (unsigned char*)addr >= heap && (unsigned char*)addr + len <= mem_brk) {
...
void hprobe(void* ptr, int offset, size_t count) {
  ...
  for (iptr = cptr_hi; iptr >= cptr_lo; iptr--) printf("%.2x", (unsigned)mem_read((void*)iptr, 1));
  printf("\n");
}
...
static void print_stats() {
  ...
  if (!show_stats || vbytes == 0 || stats_printed) return;
  if (sparse) {
    size_t ppages = num_pages - num_free_pages;
    size_t pbytes = ppages * SPARSE_PAGE_SIZE;
    printf(
        "Allocated %zu/%zu pages (%zu bytes) to cover %zu heap bytes (%.4f%% density).  Max "
        "address = %p\n",
...
static void* get_mem(const void* addr) {
  ...
  while (block && block->id != id) block = block->next;
  if (!block) {
