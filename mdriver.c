...
static bool add_range(range_set_t* ranges, char* lo, size_t size, const trace_t* trace, int opnum,
                      int index);
...
static trace_t* read_trace(stats_t* stats, const char* tracedir, const char* filename);
...
static void unix_error(const char* fmt, ...) __attribute__((format(printf, 1, 2), noreturn));
static void app_error(const char* fmt, ...) __attribute__((format(printf, 1, 2), noreturn));
 ...
 */
static void run_tests(int num_tracefiles, const char* tracedir, char** tracefiles,
...
int main(int argc, char** argv) {
  ...
  if (num_global_tracefiles == 0) {
    ...
    if (sparse_mode & !run_libc) {
      for (i = 0; default_giant_tracefiles[i]; i++) add_tracefile(default_giant_tracefiles[i]);
    }
    for (i = 0; default_tracefiles[i]; i++) add_tracefile(default_tracefiles[i]);
  }
  ...
  min_throughput = ref_throughput * (checkpoint ? MIN_SPEED_RATIO_CHECKPOINT : MIN_SPEED_RATIO);

  max_throughput = ref_throughput * (checkpoint ? MAX_SPEED_RATIO_CHECKPOINT : MAX_SPEED_RATIO);
  ...
  run_tests(num_global_tracefiles, tracedir, global_tracefiles, mm_stats, &speed_params);
  ...
  if (errors == 0) {
    ...
    /* p1 - utilization */
    p1 = UTIL_WEIGHT * compute_scaled_score(avg_mm_util, MIN_SPACE, MAX_SPACE);
    p1_checkpoint = UTIL_WEIGHT_CHECKPOINT *
                    compute_scaled_score(avg_mm_util, MIN_SPACE_CHECKPOINT, MAX_SPACE_CHECKPOINT);

    /* p2 - throughput index */
    p2 = (1.0 - UTIL_WEIGHT) *
         compute_scaled_score(avg_mm_geom_throughput, min_throughput, max_throughput);
    ...
    if (!sparse_mode) {
      ...
      if (checkpoint) {
        printf("Checkpoint Perf index = %.1f (util) + %.1f (thru) = %.1f/100\n",
               p1_checkpoint * 100, p2_checkpoint * 100, perfindex_checkpoint);
      } else {
  ...
  if (autograder) {
    sprintf(autoresult,
            ...
            score, score, 0.0, avg_mm_geom_throughput, avg_mm_util * 100);
    printf("%s\n", autoresult);
  }
  exit(0);
}
 ...
 ****************************************************************/
static void add_tracefile(char* trace) {
  global_tracefiles = realloc(global_tracefiles, (num_global_tracefiles + 1) * sizeof(char*));
...
static bool add_range(range_set_t* ranges, char* lo, size_t size, const trace_t* trace, int opnum,
  ...
  if (!IS_ALIGNED(lo)) {
    malloc_error(trace, opnum, "Payload address (%p) not aligned to %d bytes", lo, ALIGNMENT);
    return false;
  }
  ...
  if ((lo < (char*)mem_heap_lo()) || (lo > (char*)mem_heap_hi()) || (hi < (char*)mem_heap_lo()) ||
      (hi > (char*)mem_heap_hi())) {
    malloc_error(trace, opnum, "Payload (%p:%p) lies outside heap (%p:%p)", lo, hi, mem_heap_lo(),
                 mem_heap_hi());
    return false;
  }
  ...
  if (prev && lo <= prev->hi) {
    malloc_error(trace, opnum, "Payload (%p:%p) overlaps another payload (%p:%p)\n", lo, hi,
                 prev->lo, prev->hi);
    return false;
  }
  if (next && hi >= next->lo) {
...
static void randomize_block(trace_t* traces, int index) {
  ...
  if (size == 0) return;
  fsize = size;
  if (fsize > maxfill) fsize = maxfill;
  ...
  for (i = 0; i < fsize; i++) {
    mem_write(&block[i], random_data[(base + i) % RANDOM_DATA_LEN], sizeof(randint_t));
  }
}
...
static bool check_index(const trace_t* trace, int opnum, int index) {
  ...
  if (ngarbled != 0) {
    malloc_error(trace, opnum,
                 ...
                 "starting at byte %zu",
                 index, &block[firstgarbled], ngarbled, randint_t_name, (ngarbled > 1 ? "s" : ""),
...
static trace_t* read_trace(stats_t* stats, const char* tracedir, const char* filename) {
  ...
  if ((trace->block_rand_base = calloc(trace->num_ids, sizeof(*trace->block_rand_base))) == NULL)
  ...
  for (i = 0; i < trace->num_ops; i++) {
    ...
    switch (trace->ops[i].type) {
      case ALLOC: /* mm_malloc */
         ...
         */
        if (add_range(ranges, p, size, trace, i, index) == 0) return false;
      ...
      case REALLOC: /* mm_realloc */
        ...
        if (size > 0) {
          if (add_range(ranges, newp, size, trace, i, index) == 0) return false;
        }
...
static double eval_mm_util(trace_t* trace, int tracenum) {
  ...
  mem_reset_brk();
  if (!mm_init()) app_error("trace %d: mm_init failed in eval_mm_util", tracenum);
...
static void eval_mm_speed(void* ptr) {
  ...
  mem_reset_brk();
  if (!mm_init()) app_error("mm_init failed in eval_mm_speed");
...
static void printresults(int n, stats_t* stats, sum_stats_t* sumstats) {
  ...
  for (i = 0; i < n; i++) {
    if (stats[i].valid) {
      ...
      if (tab_mode) {
        ...
        if (stats[i].weight == WNONE || stats[i].weight == WALL || stats[i].weight == WUTIL)
      ...
      if (tab_mode) {
        ...
        if (stats[i].weight == WNONE || stats[i].weight == WALL || stats[i].weight == WPERF)
  ...
  if (errors == 0) {
    ...
    if (sparse_mode) sumsecs = 0;
    if (tab_mode) {
      // "valid\tthru?\tutil?\tutil\tops\tmsecs\tKops\ttrace"
      printf("Sum\t%d\t%d\t%.1f\t%.0f\t\%.2f\n", sum_perf_weight, sum_util_weight, sumutil * 100.0,
             sumops, sumsecs * 1000.0);
      printf("Avg\t\t\t%.1f\t\t\t\n", util * 100.0);
    } else {
      printf("%2d %2d  %7.1f%%%8.0f%10.3f\n", sum_util_weight, sum_perf_weight, util * 100.0,
             sumops, sumsecs * 1000.0);
    }
...
static double lookup_ref_throughput(bool checkpoint) {
  ...
  while (fgets(buf, MAXLINE, tfile) != NULL) {
    int t = cparse(buf, tokens);
    if (t < 3) continue;
    if (strcmp(tokens[0], cpu_type) == 0 && strcmp(tokens[1], bench_type) == 0) {
      tput = atof(tokens[2]);
      found = true;
      break;
    }
...
static double measure_ref_throughput(bool checkpoint) {
  ...
  if (ltput > 0) return ltput;
  char buf[MAXLINE];
  char cmd[MAXLINE];
  char* fname = gen_file_name("./tput_%.8x.txt", buf, MAXLINE);
  float t;
  sprintf(cmd, "%s > %s", checkpoint ? REF_DRIVER_CHECKPOINT : REF_DRIVER, fname);
  if (verbose > 1) {
    printf("Executing '%s'\n", cmd);
  }
  if (system(cmd) != 0) {
