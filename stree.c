...
void tree_free(tree_t* tree, free_fun_t free_fun) {
  if (tree->root) free_subtree(tree->root, free_fun);
  free(tree);
}
...
bool tree_insert(tree_t* tree, tkey_t key, void* record) {
  ...
  while (z) {
    ...
    if (key == z->key) /* Already have key in tree */
      return false;
    tree->comparison_count++;
    if (key > z->key)
      z = z->right;
